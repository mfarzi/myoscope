function [params, rmse, costFunc, exitFlag] = fitRician(obj, data, schemefile, varargin)
...
fitter = optimizer('algorithm', 'interior-point','fevalTol',1e-8,'prettyPrint',false); % an "optimizer" object for fitting model parameters
%sigma = linker('sigma', 'bounded', 0.01,0.15);
fitter.setfeval(@(x, s) obj.FevalRician(x, s, schemefile))
fitter.setfjac(@(x,s) obj.FgradRician(x, s, schemefile));
...
if nWorkers==0 || N==1
    ...
    for n=1:N
        ...
        x0 = [0.1; obj.links.initx()];
        % fit model to sig
        [x, cost, flag] = fitter.run(x0, sig);
        sigma = x(1)^2+0.0001;%sigma.map(x(1));
        p = obj.links.map(x(2:end));
        %err = cost;%sqrt(cost/nScheme);
        % store results
        params(:,n) = p;
        rmse(n) = sigma;%sqrt(sigma2);
        costFunc(n) = cost;
        exitFlag(n) = flag;
        % print results
        if progress
...
if nreps>1
    costFunc = reshape(costFunc, [nreps, nVoxels]);
    [costFunc, idx] = min(costFunc);
