classdef tensorBD < compartment
    ...
    methods 
        function obj = tensorBD(varargin) 
            %TENSOR Construct Function.
            %   tensorBD() construct an object with default set of parameters
            %   DT = eye(3)*1e-9
            %
            %   tensorBD(params) construct an object with given initial 
            %   params; [s0; diffPar; diffPerp1; diffPerp2; theta; phi; alpha]
            %
            %   tensorBD(..., <Parameter>, <Value>, ...) allow
...
%             obj.links.addConstraint('p2>=p3');
        ...
        function set(obj, varargin)
            ...
            if nargin==1
                ...
                p.addParameter('diffPerp' , obj.diffPerp    , @obj.validationFCN_diffPar);
                ...
                obj.updateParams([obj.s0; obj.diffPar; obj.diffPerp; ...
                                  obj.theta; obj.phi; obj.alpha; ...
                                  obj.kappa1; obj.kappa2]);             
            end % of if nargin==0
        end % of set
        ...
        function rotateAxis(obj)
            ...
            obj.modelParams(4:6) = [obj.theta; obj.phi; obj.alpha]; 
        end % of rotateAxis
        ...
        function [dPar, dPerp1, dPerp2] = getDiffusivity(obj)
            dPar = obj.ecDiffPar;
            dPerp1 = obj.ecDiffPerp1;
            dPerp2 = obj.ecDiffPerp2;
        end
        ...
        function setEcDiffParmas(obj)
            ...
            obj.ecDiffPar = sum(((obj.diffPar-obj.diffPerp)*(u1(1)*x+u1(2)*y+u1(3)*z).^2 + obj.diffPerp).*probs.*sin(thisTheta), 1);
            obj.ecDiffPerp1 = sum(((obj.diffPar-obj.diffPerp)*(u2(1)*x+u2(2)*y+u2(3)*z).^2 + obj.diffPerp).*probs.*sin(thisTheta), 1);
            obj.ecDiffPerp2 = obj.diffPar+ 2*obj.diffPerp - ...
                              obj.ecDiffPar - obj.ecDiffPerp1;
        end
    ...
    methods (Access = protected)
        ...
        function jac = jacobian(obj, scheme)
            ...
            gEcDiffPar_gDiffPerp = sum((1-(obj.U(1,1)*x+obj.U(2,1)*y+obj.U(3,1)*z).^2).*probs.*sin(thisTheta), 1);
            gEcDiffPerp1_gDiffPerp = sum((1-(obj.U(1,2)*x+obj.U(2,2)*y+obj.U(3,2)*z).^2).*probs.*sin(thisTheta), 1);
            ...
            gDPar_gU1 = sum(2*(obj.diffPar-obj.diffPerp)*(obj.U(1,1)*x+obj.U(2,1)*y+obj.U(3,1)*z).*[x,y,z].*probs.*sin(thisTheta), 1);
            gDPar_gU2 = zeros(1,3);
            gDPar_gU3 = zeros(1,3);
            ...
            gDPerp1_gU2 = sum(2*(obj.diffPar-obj.diffPerp)*(obj.U(1,2)*x+obj.U(2,2)*y+obj.U(3,2)*z).*[x,y,z].*probs.*sin(thisTheta), 1) + ...
                          sum(((obj.diffPar-obj.diffPerp)*(obj.U(1,2)*x+obj.U(2,2)*y+obj.U(3,2)*z).^2 + obj.diffPerp).*sin(thisTheta).*gP_gU2, 1);
            gDPerp1_gU3 = sum(((obj.diffPar-obj.diffPerp)*(obj.U(1,2)*x+obj.U(2,2)*y+obj.U(3,2)*z).^2 + obj.diffPerp).*sin(thisTheta).*gP_gU3, 1);
            ...
            gDPar_gK1 = sum(((obj.diffPar-obj.diffPerp)*(obj.U(1,1)*x+obj.U(2,1)*y+obj.U(3,1)*z).^2+ obj.diffPerp).*gP_gK1.*sin(thisTheta), 1);
            gDPerp1_gK1 = sum(((obj.diffPar-obj.diffPerp)*(obj.U(1,2)*x+obj.U(2,2)*y+obj.U(3,2)*z).^2+ obj.diffPerp).*gP_gK1.*sin(thisTheta), 1);
            gDPerp2_gK1 = - gDPar_gK1 - gDPerp1_gK1;
            
            jac(:,7) = gf_gEcDiffPar.*gDPar_gK1 + gf_gEcDiffPerp1.*gDPerp1_gK1 + ...
                       gf_gEcDiffPerp2.*gDPerp2_gK1;
            
            gDPar_gK2 = sum(((obj.diffPar-obj.diffPerp)*(obj.U(1,1)*x+obj.U(2,1)*y+obj.U(3,1)*z).^2+ obj.diffPerp).*gP_gK2.*sin(thisTheta), 1);
            gDPerp1_gK2 = sum(((obj.diffPar-obj.diffPerp)*(obj.U(1,2)*x+obj.U(2,2)*y+obj.U(3,2)*z).^2+ obj.diffPerp).*gP_gK2.*sin(thisTheta), 1);
        ...
        function updateParams(obj, p)
            ...
            obj.diffPerp    = p(3);
            obj.theta       = p(4);   % elevation angle
            obj.phi         = p(5);   % azimuth angel
            obj.alpha       = p(6);   % angele beween v1_zrot and v2
            obj.kappa1      = p(7);
            obj.kappa2      = p(8);
            obj.modelParams = p;
